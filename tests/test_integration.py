...
# Force matplotlib to use headless mode by setting environment variables
...
def test_csv_output():
    ...
    with open(csv_file) as f:
        ...
        # Verify last row
        last_row = rows[-1]
        # With daily compounding, there may be a small remaining balance since the 
        # monthly payment calculation is based on monthly compounding
        remaining_principal = float(last_row[3])
        assert remaining_principal < 5000, f"Remaining principal should be small, got {remaining_principal:.2f}"
