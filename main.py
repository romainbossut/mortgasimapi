    ...
    for m in range(total_months):
        ...
        if m < fixed_term_months:
            ...
            if not mortgage_paid_off and (
                m == fixed_term_months
                or (
                    idx > 0 and mortgage_rate_curve[idx] != mortgage_rate_curve[idx - 1]
                )
            ):
        ...
        interest_for_month = (
            0 if mortgage_paid_off else principal * monthly_mortgage_rate
        )
        ...
        principal_repaid = (
            total_payment - interest_for_month if not mortgage_paid_off else 0
        )
    ...
    try:
        ...
        for pair in pairs:
            ...
            if month < 0 or month >= term_months:
                raise ValueError(
                    f"Month {month} is outside the mortgage term (0-{term_months-1})"
                )
            if amount < 0:
        ...
        print(
            "Expected format: 'month:amount,month:amount' (e.g., '18:20000,19:10000')"
        )
